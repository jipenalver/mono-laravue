<template>
    ...
    <main id="main"
        ...
        <div class="pagetitle">
            <h1>
                <i class="bi bi-card-checklist"></i> List of Tasks
            </h1>
            <br>
            <nav>
                <ol class="breadcrumb">
                                            ...
                                            <ul class="dropdown-menu">
                                                <li>
                                                    <a class="dropdown-item with-pointer"
                                                       @click="setTaskComplete(task.id)">
                                                        <i class="bi bi-card-checklist"></i> Mark as Complete
                                                    </a>
                                                </li>
                                                <li>
                                                    <a class="dropdown-item with-pointer"
                                ...
                                <ul class="pagination justify-content-center">
                                    <template v-for="link in tasks.links">
                                        ...
                                        <li class="page-item with-pointer"
                                            ...
                                            <a @click="getTasks(filterSort, link.url)"
                                               class="page-link">
...
const { task, tasks, createdTask, getTasks, selectTask, storeTask, updateTask, destroyTask, setCompleteTask, archiveTask, storeTaskAttachment, storeTaskTags } = useTasks()
...
// Create Task
const createAction = () => {
    // When I click the Create Task button, I want to see a modal with a task form.
    resetAction()
    resetForm()
    action.type = "Create"
}
...
const setTaskComplete = async (id) => {
    try {
        // When I click the Mark as Complete button, I want my task to be removed from its current list and be moved automatically to the Completed tasks list.
        await setCompleteTask(id)
        await getTasks(filterSort)
        toast.success('Set task as completed!', { position: 'top-right', duration: 5000 })
    } catch (e) {
        // If an error occurs, an error message must be displayed.
        if (e.response.status >= 400) {
            toast.error('An error occur during the process.', { position: 'top-right', duration: 5000 })
