<template>
    ...
    <main id="main"
        ...
        <div class="pagetitle">
            <h1>
                <i class="bi bi-card-checklist"></i> Completed Tasks
            </h1>
            <br>
            <nav>
                <ol class="breadcrumb">
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-2">
                                    <button type="button"
                                            ...
                                            @click="getTasksCompleted(filterSort)">
                                            ...
                                            <ul class="dropdown-menu">
                                                <li>
                                                    <a class="dropdown-item with-pointer"
                                                       @click="setTaskIncomplete(task.id)">
                                                        <i class="bi bi-file-x-fill"></i> Mark as Incomplete/Todo
                                                    </a>
                                                </li>
                                                <li>
                                                    <a class="dropdown-item with-pointer"
                                                       ...
                                                       @click="dueDateAction(task.id)">
                                                        <i class="bi bi-calendar-date"></i> Set Due Date
                                                    </a>
                                                </li>
                                                <li>
                                                    <a class="dropdown-item with-pointer"
                                                       @click="setTaskArchive(task.id)">
                                ...
                                <ul class="pagination justify-content-center">
                                    <template v-for="link in tasks.links">
                                        ...
                                        <li class="page-item with-pointer"
                                            ...
                                            <a @click="getTasksCompleted(filterSort, link.url)"
                                               class="page-link">
...
const { task, tasks, createdTask, getTasksCompleted, selectTask, storeTask, updateTask, destroyTask, setIncompleteTask, archiveTask, storeTaskAttachment, storeTaskTags } = useTasks()
...
const saveTask = async () => {
    ...
    try {
        ...
        await getTasksCompleted(filterSort)
    } catch (e) {
...
const setTaskIncomplete = async (id) => {
    try {
        // When I click the Mark as Incomplete/Todo button, I want my task to be removed from its current list and be moved automatically to the Todo tasks list.
        await setIncompleteTask(id)
        await getTasksCompleted(filterSort)
        toast.success('Set task as incomplete todo!', { position: 'top-right', duration: 5000 })
    } catch (e) {
        // If an error occurs, an error message must be displayed.
        if (e.response.status >= 400) {
            toast.error('An error occur during the process.', { position: 'top-right', duration: 5000 })
...
const setTaskArchive = async (id) => {
    try {
        // The task should be removed from the list when archived.
        await archiveTask(id)
        await getTasksCompleted(filterSort)
        toast.success('Successfully archived task!', { position: 'top-right', duration: 5000 })
    } catch (e) {
        // If an error occurs, an error message must be displayed.
        if (e.response.status >= 400) {
            toast.error('An error occur during the process.', { position: 'top-right', duration: 5000 })
